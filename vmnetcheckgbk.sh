...
curr_conn_net()
{
	curr_conn_tcp=$(netstat -anp|grep ^tcp|grep -v :::|grep ESTABLISHED |sort -rn -k 3|awk '{print $1,"\t入队列:",$2,"\t出队列:",$3,"\t  本地IP:",$4,"\t\t远程IP:",$5,"\t端口/进程:",$NF}')
	if [ -n "${curr_conn_tcp}" ];then
		echo -e "\r\033[K${curr_conn_tcp}\n\n"|grep -v 132|head -15
	fi
	curr_conn_udp=$(netstat -anp|grep ^udp|grep -v ::|grep ESTABLISHED |sort -rn -k 3|awk '{print $1,"\t入队列:",$2,"\t出队列:",$3,"\t  本地IP:",$4,"\t\t远程IP:",$5,"\t端口/进程:",$NF}')
